...
double v_norm(tf::StampedTransform transform)
{
  double norm;
  norm = transform.getOrigin().x()*transform.getOrigin().x()
      ...
      if(new_msg_flat==true)
        {
          ...
          // vector<double> d_l = v_sub(transform_v[4],transform_v[3]);
          //vector<double> d_r = v_sub(transform_v[7],transform_v[6]);      
          ...
          for(int i=0; i<transform_v.size(); i++)
            {
              double norm = v_norm(transform_v[i]);
              fout<<norm<<",";
